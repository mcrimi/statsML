...
#Apply natural logarithm to Y, as indicated in the assgignment
#"..and the response variable ln(y)"
Y<- log(Yraw)
...
#CROSS VALIDATION #
#Setting nfolds to 5 as it seems to be the standard (20% of data for testing)
nfolds=5
...
#Using this particular seed (124), the best performing model seems to be fitPrunedTreeVS.
summary(fitPrunedTreeVS)
...
#while x8 seems to be irrelevant.
#Also: what is the nfold that rpart uses?


#################################################
#Appendix:
#Model performance comparisson over nstab runs  #
#################################################
AICWins<-0
CARTWins<-0
nstab<-20
...
for (i in 1:nstab){
  ...
  nfolds<-5
  #CART
  fitVSURF<-VSURF(x=X,y=Y,mtry = 10,verbose = FALSE)
  fitMaxTreeVS=rpart(Y~.,data=X[c(fitVSURF$varselect.pred)], control=rpart.control(minsplit=2,cp=0))
  fitMaxTreeVS.CP <- printcp(fitMaxTreeVS)
  fitMaxTreeVS.CV <- fitMaxTreeVS.CP[,4]
  mincverr= as.numeric(which.min(fitMaxTreeVS.CV))
  ...
  #AIC
  fitBoth_AIC <- stepAIC(fitGLM, direction = "both", trace = FALSE) 
  cvfitBoth_AIC<-CVlm(data=X,form.lm=fitBoth_AIC, m=ceiling((runif(1,1,5))), seed=seed)
  fitBoth_AIC.CVMSE<-attributes(cvfitBoth_AIC)$ms
